...
> **Provenance note, added by adversarial re-measurement.** That sha256 is not
> independently checkable: neither the dump file nor the script that produced
> it is committed, and the dump's format is not stated, so the digest cannot be
> regenerated by anyone reading this. The *claim* it stands for was re-derived
> from scratch and holds — 32 `(phase, id, status, reason)` verdicts dumped
> against a fixture repo on a fixed temp path, once with `main` 3df724d's `src`
> on `sys.path` and once with this tree's, come back byte-identical (a
> different dump shape, so a different digest:
> `14687e1ea1661bc785448df44d4599c35c1484fc4aa334826d11df1b15e8e12e`, equal
> across both trees). Future negative controls of this kind should commit the
> probe, or quote no digest.

### SV-2-PHASE-EXIT — `5a2b3c4`

> **CORRECTION, 2026-08-29, from an adversarial re-measurement.** This section
> originally read: *"The printed fraction and the exit code both came from the
> results the run collected, so numerator and denominator agreed by
> construction and the comparison carried no information."* That baseline was
> written from reasoning rather than from a run, and it is false. It was
> pre-registered in the same false form at
> `SILENT_VANISHING_PREREGISTRATION.md:24` ("Eleven of twelve checks running
> prints `11/11 PASS` and exits `0`"). The pre-registration is left as written
> — rewriting a prereg after the fact destroys the only thing it is for — and
> the error is corrected here, where the results live. See "The baseline that
> was wrong" below.

`tests/test_phase_denominator.py` — **5 passed, 1.75 s** (was 4; one control
...
| positive | `_run_phase` made lossy | `cmd_check` exits **1**, stderr names both sides: `11 result(s) for 12 declared check(s)` — but see the reachability note below |
| positive | a registered check returns a `CheckResult` carrying the **wrong `check_id`**, `_run_phase` untouched | `cmd_check` exits **1**, stderr `13 result(s) for 12 declared check(s)` |
| negative | complete readiness run, fixture repo | no `INCOMPLETE` on stderr; the exit code is the ladder's |

**The baseline that was wrong.** Measured, not described — `main` 3df724d, R5
removed from `_REGISTRY`, readiness run against a fixture repo on a temp path:

    R5     -          not run
    READINESS: 1/12 PASS  ESCALATED=1  NA=9        exit 3

Three specifics, each checkable:

1. The denominator was **already** `PHASE_ORDER`'s. `main` 3df724d
   `cli.py:109` reads `total = len(PHASE_ORDER[phase])`, and `phase_ids(phase)`
   returns `list(PHASE_ORDER[phase])`. That half of SV-2 is a no-op refactor,
   not a behaviour change.
2. The lost row was **not invisible**. `core/table.py:48` already rendered a
   declared id with no result as `-  not run`.
3. The run exited **3**, not `0`, and printed `1/12`, never `11/11`. `11/11` is
   in fact unreachable on that build, since `total` never shrinks.

**The defect that is real**, and that the two guards do fix: eleven statuses
were counted beside a denominator of twelve, and the ladder answered `3` —
"unmeasured, stale or awaiting sign-off" — for a registry that could not
account for its own declared parts. That is an instrument fault, and it must
exit `1`. It now does.

**Reachability of the `cmd_check` guard, stated honestly.** Since `_run_phase`
backfills every declared id, `observed < expected` cannot arise from production
code; the original positive control reaches it only by replacing `_run_phase`
itself. `observed > expected` **is** production-reachable — `_run_phase` does
not overwrite `result.check_id`, so a check function returning a mislabelled
result leaves its declared id unproduced, gets a backfilled FAIL, and arrives
one row long. `test_lost_check_exits_one_from_a_production_shaped_defect`
forces exactly that, and it fails when the guard is disabled.
