...
| no threshold, range or floor edited | `git diff 24f23b8..HEAD -- src/ \| grep -E "^[+-]\s*(MIN_HOLDOUT_GROUPS\|MAX_REASON\|MAX_COVERAGE_TOL\|MAX_METRIC_TOL\|MIN_COVERAGE_N)\s*="` is **empty**. (The looser `.*` form matches one line: the `# MAX_REASON = 400 characters` comment in D6 explaining why the NA guidance was put back unchanged. Named here so nobody has to wonder why the two greps disagree.) | holds |
| no existing test file modified | `git diff 24f23b8..HEAD --name-status -- tests/` is `A tests/test_track_aware_splits.py`, one added file and nothing else | holds |
